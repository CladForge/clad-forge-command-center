...
const TABLE_COLUMNS = {
  clients: ['id','name','company','email','phone','industry','status','notes','value','created_at','created_by','brand_colors','brand_fonts','brand_tone','brand_logo_url','website','company_size','contacts','address'],
...
export function useSupabaseData() {
  ...
  const setFinanceEntries = useCallback(
    makeSetter(setFinanceEntriesState, 'finance_entries', { labelField: 'description', entityLabel: 'finance entry', logActivity: false }),
    [addActivity]
  );

  const setTaxPayments = useCallback(
    makeSetter(setTaxPaymentsState, 'tax_payments', { labelField: 'quarter', entityLabel: 'tax payment', logActivity: false }),
    [addActivity]
  );
  ...
  const setMarkupSets = useCallback(
    makeSetter(setMarkupSetsState, 'markup_sets', { labelField: 'name', entityLabel: 'markup set', icon: 'layers' }),
    [addActivity]
  );
  ...
  const reloadAdminScreenshots = useCallback(async () => {
    ...
    const [{ data: ssData }, { data: pinsData }, { data: setsData }] = await Promise.all([
      supabase.from('app_screenshots').select('*').order('created_at', { ascending: false }),
      supabase.from('annotation_pins').select('*').order('created_at', { ascending: true }),
      supabase.from('markup_sets').select('*').order('created_at', { ascending: false }),
    ]);
    ...
    if (setsData) setMarkupSetsState(setsData.map(snakeToCamel));
  }, []);
